...
#include <stdint.h>
/* Active sample count for current SPWM tables */
...
void MX_TIM7_Init(void)
{
  ...
  htim7.Instance = TIM7;
  htim7.Init.Prescaler = 62500-1;
  htim7.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim7.Init.Period = 480-1;
...
// hdma_tim3_ch1.XferCpltCallback = HAL_DMA_XferCpltCallback;
// we need to insert this inside the MSP_TIMx_INIT
// i dont know why the callback is not handled by default


void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef* htim)
{

    /* Only process TIM1, Channel 1 (master) */
    if (htim->Instance != TIM1) return;
    if (htim->Channel != HAL_TIM_ACTIVE_CHANNEL_1) return;

    /* Nothing to do if no pending swap */
    if (!spwm_swap_request_f) return;

    /* Reentrancy guard */
    static uint8_t in_swap = 0;
    if (in_swap) return;
    in_swap = 1;

    /* Stop both PWM+DMA streams (ignore status) */
    HAL_TIM_PWM_Stop_DMA(&htim1, TIM_CHANNEL_1);
    HAL_TIM_PWM_Stop_DMA(&htim1, TIM_CHANNEL_2);
    HAL_TIMEx_PWMN_Stop(&htim1, TIM_CHANNEL_1);
    HAL_TIMEx_PWMN_Stop(&htim1, TIM_CHANNEL_2);

    spwm_start_request_f = 1;
    spwm_swap_request_f = 0;
    in_swap = 0;
}
